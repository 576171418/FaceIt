...
class FaceView: UIView {
    ...
    @IBInspectable
    var mouthCurvature: Double = 1.0 { didSet { setNeedsDisplay() }} //1是最开心，-1是最不开心
    ...
    //滑动手势处理操作
    @objc func changeHappines(byReactingTo swipeRecognizer: UIPanGestureRecognizer) {
        
    }
    
    private var skullRadius: CGFloat {
        //只读属性，计算脸的半径，半径为屏幕宽度/2再乘比例
    ...
    private func pathForMouth() -> UIBezierPath {
        ...
        let cp1 = CGPoint(x: start.x + mouthRect.width / 3, y: start.y + simleOffset)
        let cp2 = CGPoint(x: end.x - mouthRect.width / 3, y: start.y + simleOffset)
